...
public sealed class PasswordAttribute : ValidationAttribute
{
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        if (password.Length < PasswordLimits.MinLength) return new ValidationResult(ErrMsgTooShort);
        
        if (password.Length > PasswordLimits.MaxLength) return new ValidationResult(ErrMsgTooLong);
