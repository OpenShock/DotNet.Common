...
public sealed class EmailAddressAttribute : ValidationAttribute
{
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        if (email.Length < EmailAddressLimits.MinLength) return new ValidationResult(ErrMsgTooShort);
        
        if (email.Length > EmailAddressLimits.MaxLength) return new ValidationResult(ErrMsgTooLong);
